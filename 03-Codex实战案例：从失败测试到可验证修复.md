...
## 开篇
前两篇已经讲过 Codex 的入口、配置、权限和复用能力，这一篇只保留实战中最常用的部分：怎么让 Codex 稳定修 bug，怎么在提交前做安全检查，以及哪些提示词可以直接复制使用。
你可以把它当成一张工作台旁边的速查卡。遇到问题时，不必从头组织语言，按下面的结构发给 Codex 就够了。
...
这段提示词的关键，不是写得长，而是把任务边界说清楚：问题是什么、从哪里查、不能做什么、怎样算完成。

![Codex Debug 闭环](images/07-debug-loop-cn.jpg)

## 二、排查时盯住三件事
让 Codex 排查问题时，不要只让它“看一下”。你可以直接要求它回答三个问题：
...
## 结尾
Codex 实战最重要的不是“让它一次猜对”，而是把过程管住：先复现，限制改动范围，最后拿验证结果说话。
把这篇里的几段提示词存起来，日常修 bug、提 PR、做提交前检查时直接套用，Codex 会稳定很多。
