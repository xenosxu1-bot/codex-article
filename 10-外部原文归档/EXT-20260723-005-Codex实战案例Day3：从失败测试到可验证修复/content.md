...
前两篇讲过入口、配置、权限和复用能力。这一篇只保留实战中最常用的部分：修 bug、提交前安全检查、常用提示词，遇到问题时，直接套下面的结构发给 Codex。
...
Codex 修 bug 要管住过程：先复现，限制改动范围，最后用验证结果收尾，日常修 bug、提 PR、做提交前检查时，直接复用上面的模板。
