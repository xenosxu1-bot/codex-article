...
| 主流 Skill | 适合任务 | 用法提示 |
| --- | --- | --- |
| guizang-ppt-skill | 汇报、课程、分享型 PPT | 先给大纲和受众，再让它生成页面结构。 |
| guizang-social-card-skill | 小红书、公众号封面和社媒卡片 | 先固定尺寸、主标题和风格，再批量出图。 |
| awesome-gpt-image-2 | 封面、海报、产品图和人物图提示词 | 用来沉淀视觉模板，而不是每次重新想提示词。 |
| Humanizer-zh | 中文改写、去套话、去翻译腔 | 初稿完成后再用，避免观点被改散。 |
| Deep-Research-skills | 长文调研、报告汇总、资料复盘 | 要先限定来源、范围和交付格式。 |
| anything-to-notebooklm | 网页、视频、PDF 到播客、PPT、思维导图 | 适合把已有材料变成可学习资料。 |
| wewrite | 选题、正文、SEO、配图、排版和发布草稿 | 适合内容团队沉淀一套固定生产线。 |
| Youtube-clipper-skill | 长视频拆短视频、高光和字幕 | 先定义目标平台和片段节奏。 |
| oh-story-claudecode | 小说、网文选题、人物和结构拆解 | 适合做创作前的结构分析。 |
| marketingskills | SEO、品牌定位、用户研究、广告和邮件营销 | 适合营销动作标准化和复盘。 |

Skill 的价值不是多写一段提示词，而是让 Codex 每次都按同一套流程执行。

## 五、先做这个决策树：插件、Skill、MCP 到底用哪个

如果你分不清插件、Skill 和 MCP，可以先按下面的问题判断。

如果问题是“我每次都要重复同一套写作、检查或交付步骤”，优先用 Skill。它适合沉淀流程，比如公众号文章检查、PDF 生成、表格分析。

如果问题是“我需要把一组能力安装给自己或团队”，考虑插件。插件更像能力包，可以包含 Skill、工具配置、资源和元数据。

如果问题是“我需要连接外部系统、实时数据或私有账号”，考虑 MCP 或连接器。比如读数据库、查内部工单、访问云盘、调用企业系统，都不应该靠模型记忆。

一句话：流程复用选 Skill，能力分发选插件，外部连接选 MCP。先用最小方案跑通，再决定要不要升级。
