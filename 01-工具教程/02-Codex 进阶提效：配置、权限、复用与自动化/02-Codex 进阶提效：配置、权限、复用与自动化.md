...
![图：Codex 进阶提效：配置、权限、复用与自动化的阅读路径](assets/figures/02-Codex 进阶提效：配置、权限、复用与自动化-正文插图01.png)
...
## 二、先把安全边界讲清楚：Sandbox 和 Approval
...
## 四、`config.toml`：让 Codex 每次都按同一套方式启动
...
## 十一、Automations 和 Subagents：让 Codex 处理长期和并行问题
