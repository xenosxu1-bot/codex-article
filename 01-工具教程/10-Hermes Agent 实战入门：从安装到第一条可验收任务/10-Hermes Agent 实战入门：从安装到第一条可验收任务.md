...
很多人第一次跑通 Agent 后，只留下终端截图和一句“能用了”。这不算真正完成入门，因为下一次换目录、换模型、换同事或换网络环境时，问题仍然会重新发生。

建议把第一次成功补成一个小型交接包，至少包含四份文件：

| 文件 | 用途 | 最低要求 |
| --- | --- | --- |
| `notes/task-card.md` | 说明本次任务怎么交代 | 目标、输入、边界、输出、验收、停止条件齐全 |
| `output/project-review.md` | 保存最终交付物 | 每个关键结论能回到文件、命令或人工确认 |
| `notes/run-log.md` | 记录执行过程 | 版本、模型、工作目录、失败重试、人工确认点 |
| `notes/next-actions.md` | 给下一次任务留入口 | 哪些步骤可复用、哪些仍需人工判断 |

这四份文件的价值不在于“形式完整”，而在于让你知道：这次成功到底依赖什么、哪些地方仍不可自动化、下一次能否由别人复现。只有能被复现和交接的任务，才值得继续沉淀成 Skill、Cron 或网关入口。

如果你准备把 Hermes Agent 用到团队协作里，还可以再补三条约定：谁有权改任务卡、谁负责审批高风险命令、失败时谁负责撤销凭据或回滚文件。这样，Agent 不再只是一个能执行命令的终端助手，而是进入了可管理的工作流程。
