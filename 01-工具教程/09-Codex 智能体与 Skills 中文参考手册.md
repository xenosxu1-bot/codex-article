> 一句话结论：这是一份按任务查阅的 Codex 智能体与 Skill 手册，不需要从头读到尾；先按你的目标选入口，再跳到对应条目。

第一次接触智能体和 Skill，最容易卡在两个极端：要么只看见一长串角色名称，不知道从哪里开始；要么为了完成一个任务，把所有能力都加进来，结果边界和验证都变得模糊。

这篇不是一篇要求你顺读到底的教程，而是一份资料库入口。遇到具体任务时，先找到“我现在要完成什么”，再查看相应角色、Skill、约定和检查清单。

如果你只想跑通第一个项目任务，优先读“新手先读 10 分钟版”和“如何在任务中使用”；如果你在为团队或仓库建立长期规则，再进入 AGENTS.md、测试与交付章节。

## 零、先按任务选入口

- 刚开始使用 Codex： 先看“新手先读 10 分钟版”，只完成一个有验收标准的小任务。
- 不知道该选什么角色： 看“快速选择指南”和“通用智能体”，先按任务类型缩小范围。
- 想把重复工作交给固定能力： 看“Skills 完整目录”和“常见工作流示例”，再决定是否需要专门 Skill。
- 需要在仓库中协作： 直接跳到“项目约定：AGENTS.md、测试与交付”，确认可修改范围和验证方式。
- 遇到命令、权限或结果不确定： 看“常见问题”和“推荐实践清单”，先回到可复核的最小步骤。

## 使用方式

1. 不要把目录当学习清单。先写下当前任务、允许修改的范围和验收结果，再查对应章节。
2. 看到一个角色或 Skill 时，先判断它是否解决了你此刻的重复工作；不能解决，就不要为了“配置齐全”而启用。
3. 每次采用新能力，都用一个可回退的小任务验证。结论要能回到文件、命令输出或明确的交付物中核对。
