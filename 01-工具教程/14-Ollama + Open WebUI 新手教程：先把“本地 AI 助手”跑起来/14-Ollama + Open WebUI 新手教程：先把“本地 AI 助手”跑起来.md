...
| 你的现状 | 今天先走哪条路线 | 今天的完成信号 |
| --- | --- | --- |
| 只想确认电脑能否跑本地模型 | 只安装 Ollama | `ollama run` 能完成一次低风险对话 |
| 已有 Docker Desktop，想要浏览器聊天入口 | Ollama + Open WebUI | 浏览器可在本机打开并选到模型 |
| Docker / WSL 还没就绪 | 停在 Ollama，先不碰容器 | 记录版本、模型名和终端结果，下一次再处理 Docker |
...
**成功信号**：浏览器只能通过本机地址打开；模型下拉列表里能看到刚才下载的模型；把早餐任务发送后，回答来自同一个本地模型。
**失败时先看**：`docker ps` 是否显示容器运行、端口是否被占用、Open WebUI 到 Ollama 的连接地址是否可达。先定位一层，不要同时重装 Docker、重新下载模型和改端口。
...
把图 2 保存下来。每次提问只补齐“事实、限制、格式、验收”四项，比收藏一长串万能 Prompt 更能让本地助手真正派上用场。
