...
- 封面视觉规则、提示词、动态配色与两阶段生产流程以锁定版本的 `article-Skill` 为准；项目内历史模板仅用于追溯，不得作为新的生成规则来源。
...
- 通用文章生产标准仅以私有仓库 `article-Skill` 为准；本项目通过 `09-工具脚本/article-skill.lock.json` 锁定版本。项目内不保留 `article-wechat-skill` 快照、个人安装目录或独立规则回退来源；追溯以 `article-Skill` 的 Git 历史为准。
...
- 重复度公式为“正文五字片段 Dice 相似度 × 85% + 标题二字片段 Dice 相似度 × 15%”；所有新正式文章候选必须**低于 50%**。规范化标题或正文完全一致时直接阻止。
...
- 新增配图必须使用“文章编号-中文文章标题-用途”命名，且标题与 `07-资料与流程/文章资产登记表.md` 中的正式标题一致：封面为 `NN-中文文章标题-封面.扩展名`，正文插图为 `NN-中文文章标题-正文插图01.扩展名`；封面统一放入 `08-素材库/图片/文章封面/`，正文插图统一放入 `08-素材库/图片/正文插图/`，不为单篇文章拆独立封面目录。重命名时必须同步更新全部本地引用、README 与知识库索引。
