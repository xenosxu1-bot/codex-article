...
- 外部文章用于后续查重时，归档到 `10-外部原文归档/EXT-日期-序号-标题/`，不得占用正式文章编号、文章资产登记表或主 README 的正式文章表。账号后台原创文章同步时，先导出不含后台 token 的公开 URL 清单，再运行 `node 09-工具脚本/02-内容去重与归档/批量归档微信公众号原创.mjs --manifest <清单.json> --captured-at YYYY-MM-DD --report <同步记录.json>`。
...
- 外部原文快照是追溯与去重基线；即使与已有正式文章重复度达到或超过 50%，也必须保留并在 `07-资料与流程/04-索引与报告/内容去重报告/` 登记关联，不能因相似度跳过或删除原文。该登记不构成新正式文章的查重豁免。
- 准备新增或重写正式文章并进入文章库前，先运行 `node 09-工具脚本/02-内容去重与归档/重建内容库索引.mjs`，再运行 `node 09-工具脚本/02-内容去重与归档/检查内容重复度.mjs --candidate <候选文件>`；草稿、临时 Demo 和未入库中间稿不强制执行。
...
- 标准稿和深度稿在发布、推送或完成知识库维护前，先运行 `python 09-工具脚本/01-发布校验/文章统一检查.py --id <文章编号> --strict`；再运行 `python 09-工具脚本/01-发布校验/一键发布检查.py`。后者会串联索引重建、文章质量扫描、选题绑定、图片资产、本地链接、数量一致性、Git 格式、删除项和敏感信息检查。
...
- 准备正式入库或发布的新增、重写文章，先登记或更新 `07-资料与流程/02-选题与发布/选题库.md`，并对照 `07-资料与流程/02-选题与发布/已发布文章去重登记.md` 做查重；草稿和内部试写可暂不登记，但不得只换标题重复旧文。
...
- 正式入库或发布的新增、重写文章，按 `07-资料与流程/01-当前流程/后续文章生成方案.md` 完成必要的 Brief、正文、配图（如有）、索引和发布记录；不要求固定先后顺序。如果用户只要草稿，明确不入库、不登记为正式文章，也不提交发布记录。
- 正式文章使用两位数编号，新稿使用当前最高正式编号加 1。下架、删除或转为草稿不自动重排；仅当负责人明确批准“一次性编号迁移”时，才按 `07-资料与流程/02-选题与发布/编号变更记录.md` 中的映射执行。
- 正式入库的新文章默认采用“单篇文章包”：`分类/NN-中文标题/NN-中文标题.md` 为正文，文章专属素材放在同级 `assets/cover/` 与 `assets/figures/`，并使用“文章编号-中文文章标题-用途”命名；封面如有仅登记在 `article.json`、文章元数据和资产表中，不在正文嵌入或链接。文章专属历史文件放在本包 `assets/archive/`；未入库、下架或无法唯一归属的历史素材放入 `07-资料与流程/03-资产与核验/历史素材归档/`。资料来源、核验台账和发布记录仍集中维护在 `07-资料与流程/`，不复制进文章包。临时预览、草稿图和实验素材放入 `.tmp/`，不进入正式资产登记。重命名时必须同步更新全部本地引用、README 与知识库索引。
- 新增文章时同步更新 `README.md`、`00-知识库导航/知识库总索引.md`、`分类索引.md`、`系列索引.md`、`标签索引.md`、`07-资料与流程/03-资产与核验/文章资产登记表.md` 和 `07-资料与流程/02-选题与发布/发布记录.md`；默认使用文件名前缀编号。删除、下架或转为草稿不修改既有文章文件名、图片和元数据；只有已批准的一次性迁移，才按《编号变更记录》同步这些内容。
- 更新 README、首页或知识库索引后，必须运行 `09-工具脚本/01-发布校验/重建知识库索引.py` 或按其输出规则重建，确认展示编号与文章文件名前缀一致、链接可访问、资产登记表、元数据和正在使用的素材文件均已同步，需要时可依《编号变更记录》追溯。
- 更新 README、首页或知识库索引后，正式入库任务必须运行 `09-工具脚本/01-发布校验/重建知识库索引.py`，确认展示编号与文件名前缀一致、链接可访问、资产登记表、元数据和正在使用的素材文件已同步；批量重排时另行检查文件、图片、元数据和映射记录，并按《编号变更记录》追溯。
...
- 正式文章的编号、标题、分类、系列、标签、路径、字数和入库状态，以 `07-资料与流程/03-资产与核验/文章资产登记表.md` 为当前汇总索引；标准稿和深度稿另以 `07-资料与流程/03-资产与核验/文章元数据/NN-中文标题.json` 记录单篇素材、来源、封面与检查结果。选题状态、发布记录、图片记录和下架记录分别按 `07-资料与流程/01-当前流程/更新工作流.md` 的分工维护。
- 下架或合并文章时，必须更新 `07-资料与流程/02-选题与发布/下架文章与替代关系.md`；历史发布编号保留在记录中，需要批量重排时以 `编号变更记录.md` 为准。
- 新增、替换或清理图片后，运行 `09-工具脚本/01-发布校验/图片资产检查.py`；文章包内的正文插图必须被该正文引用并登记到 `article.json`，文章包内封面必须与该文章编号匹配且不得作为正文链接；`assets/archive/` 和 `07-资料与流程/03-资产与核验/历史素材归档/` 中的文件不得被正式正文直接引用。未归属或已下架编号残留的图片不得直接忽略。
...
- 文章专属素材放入对应文章包 `assets/`，脚本和流程文档分别放入 `09-工具脚本/`、`07-资料与流程/`；过时脚本和旧提示词模板放入 `07-资料与流程/90-历史记录/历史脚本与模板/`，未入库、下架或无法唯一归属的历史素材放入 `07-资料与流程/03-资产与核验/历史素材归档/` 并登记；不保留顶层集中素材库或整包发布归档。

## 文章质量与平台适配

- 发布版可以采用适合目标平台的任意文章结构，不强制 H1、引用式开头、固定结尾、固定加粗数量、固定表格列数、代码块转换或图片模块。
- 平台兼容检查只关注事实、版权、安全、隐私、链接和文件完整性；排版、语气、阅读节奏和视觉选择由作者或项目负责人决定。
- AI 首稿采用“后台 Brief + 作者模式 + 总编辑复审”双轮流程：Brief 约束读者场景、核心判断、反例、可复用资产和配图任务，但不得固化公开正文的文风、章节数量或句式；去模板化问题只作为非阻断编辑提醒。
...
- 正式文章使用两位数编号，新稿使用 `07-资料与流程/03-资产与核验/文章资产登记表.md` 中当前最高正式编号加 1。下架、删除或转为草稿时，不自动重排以及不批量更名已发布文章。
- 只有负责人明确批准“一次性编号迁移”时，才允许重排。先在 `07-资料与流程/02-选题与发布/编号变更记录.md` 固化旧号→新号映射、涉及资产和验收标准，再执行。
- 一次性迁移必须同步正文、正在使用的图片、正文引用、元数据、资产登记表、README/索引、本地链接和发布记录，然后运行 `python 09-工具脚本/01-发布校验/一键发布检查.py`。
- `07-资料与流程/03-资产与核验/文章资产登记表.md` 同时记录「入库状态」和「发布状态」；正式文章上线、修改或下架时先更新发布状态，再运行 `09-工具脚本/01-发布校验/重建知识库索引.py` 同步 README 的每篇文章状态列。
