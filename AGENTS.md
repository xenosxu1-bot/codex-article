...
- 正式入库的配图必须使用“文章编号-中文文章标题-用途”命名，且标题与 `07-资料与流程/文章资产登记表.md`中的正式标题一致：封面为 `NN-中文文章标题-封面.扩展名`，正文插图为 `NN-中文文章标题-正文插图01.扩展名`；封面统一放入 `08-素材库/图片/文章封面/`，正文插图统一放入 `08-素材库/图片/正文插图/`，不为单篇文章拆独立封面目录。临时预览、草稿图和实验素材放入 `.tmp/`，不进入正式资产登记。重命名时必须同步更新全部本地引用、README 与知识库索引。
...
- 新增、替换或清理图片后，运行 `09-工具脚本/图片资产检查.py`；如有正文插图，必须被正式文章引用；如有封面，必须与正式文章编号匹配，未归属或已下架编号残留的图片不得直接忽略。
