...
当项目需要使用 GPT Image 生成封面时，默认采用“GPT Image 2 生成无文字视觉底图 + 本地确定性中文叠字”的两阶段流程。图片模型只负责背景、右侧主体、图标化模块、光效和空间层次；中文主标题、副标题、短标签和最终 1792 × 764 或 2350 × 1000 成品由本地脚本完成，避免中文错字、乱码和密集小字。当前仓库默认脚本为 `09-工具脚本/gpt-image封面生成.py`，环境变量为 `OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_IMAGE_MODEL=gpt-image-2`、`OPENAI_IMAGE_SIZE`、`OPENAI_IMAGE_QUALITY`。
