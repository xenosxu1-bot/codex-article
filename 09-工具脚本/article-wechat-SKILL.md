...
当用户要求生成、优化或批量制作文章封面时，先读取 `references/visual-policy.md` 和 `references/templates/cover-prompt-template.md`。生成封面前必须先执行风格规范检查：比例、左侧标题副标题安全区、右侧主题视觉、底部 4 个能力关键词、文字量、品牌/水印/乱码风险。默认采用 V1.4 统一封面系统：2.35:1 横版，微信公众号优先输出 1792 × 764 px；左侧 55%—60% 放标题与副标题，右侧只保留 2—3 个主题视觉元素，底部放 4 个居中的能力关键词卡片或紧凑标签卡，默认只保留图标 + 关键词，整体为 2026 AI 科技产品发布会风格，并按文章内容动态调整背景色。

正式封面默认使用视觉底图与确定性中文排版叠加的两阶段方式，避免 AI 生成中文错字。封面文字必须少而强：只保留核心主标题、简短副标题和 4 个图标化能力关键词；右侧信息优先用图标、节点、进度条、箭头和卡片轮廓表达，禁止长句、密集小字和说明型信息卡。除非用户明确提供授权素材，不复制参考图中的品牌、Logo、界面、文案或具体构图；AI 视觉必须如实标记。


### GPT Image 2 封面生成流程

当项目需要使用 GPT Image 生成封面时，默认采用“GPT Image 2 生成无文字视觉底图 + 本地确定性中文叠字”的两阶段流程。图片模型只负责背景、右侧主体、图标化模块、光效和空间层次；中文主标题、副标题、短标签和最终 1792 × 764 成品由本地脚本完成，避免中文错字、乱码和密集小字。当前仓库默认脚本为 `09-工具脚本/gpt_image_cover.py`，环境变量为 `OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_IMAGE_MODEL=gpt-image-2`、`OPENAI_IMAGE_SIZE`、`OPENAI_IMAGE_QUALITY`。
...
- 正文不保留“审核记录”“图片来源”“参考资料”“发布前请复核”等内部流程区块；公开稿只可保留统一收束句：`本文发布前已完成事实、版权与格式检查。`
...
补充封面规则：左侧文字区只保留标题 + 副标题，不添加穿过文字的横线、分割线或多余价值句；右侧元素控制在 2—3 个主题视觉；底部默认只放图标 + 关键词并居中，右侧亮度不能压过左侧标题，整张图只保留一个主视觉中心。
