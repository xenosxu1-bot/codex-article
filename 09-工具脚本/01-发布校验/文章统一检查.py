...
CONFIG_PATH = TOOLS_ROOT / "03-技能接入" / "article-skill-project.json"
LOCK_PATH = TOOLS_ROOT / "03-技能接入" / "article-skill.lock.json"
...
def main() -> int:
    ...
    manifest = Path(args.manifest) if args.manifest else locate_one(f"07-资料与流程/03-资产与核验/文章元数据/{normalize_id(args.id)}-*.json", "metadata")
