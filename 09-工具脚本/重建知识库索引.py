...
def update_readme(rows):
    ...
    before = re.sub(
        ...
        f"这是一个面向 AI 工具、Codex 使用、AI Agent 工作流和个人效率方法的中文知识库。当前保留 {len(rows)} 篇整合优化版文章：重复主题已合并，短稿已补足适用边界、执行清单和验收标准；README 按文章编号升序展示，编号与文章文件名前缀、资产登记表和发布记录保持一致，已下架编号不重排。",
        before,
    )
    ...
    out = [
        ...
        "## 全部文章",
        "",
        "> 编号规则：README 使用文章文件名前缀编号，已下架编号不重排；编号与正文路径、文章资产登记表和发布记录保持一致。",
        "",
        "| 编号 | 标题 | 分类 | 系列 | 标签 | 中文字数 |",
        "| ---: | --- | --- | --- | --- | ---: |",
    ]
...
def update_home(rows):
    ...
    out = [
        ...
        "",
        f"> 更新时间：{DATE}；当前正式文章 {len(rows)} 篇。",
        "> 发布状态：全部已整合入库；首页和总索引使用文章文件名前缀编号，已下架编号不重排。",
        "",
        "## 从哪里开始",
        "",
...
def update_agents_rule():
    p = ROOT / "AGENTS.md"
    text = p.read_text(encoding="utf-8")
    new = "- 新增文章时同步更新 `README.md`、`00-知识库导航/知识库总索引.md`、`分类索引.md`、`系列索引.md`、`标签索引.md` 和 `07-资料与流程/发布记录.md`；读者可见的 README、首页和索引使用文章文件名前缀编号，已下架编号不重排；文件名、图片名、资产登记表和发布记录保持同一编号口径，避免链接和记录失真。"
    ...
    if count == 0 and new not in text:
        text = text.rstrip() + "\n" + new + "\n"
    rule = "- 更新 README、首页或知识库索引后，必须运行 `09-工具脚本/重建知识库索引.py` 或按其输出规则重建，确认展示编号与文章文件名前缀一致、链接可访问、资产登记表仍保留历史编号。"
    if rule not in text:
