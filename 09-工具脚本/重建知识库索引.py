...
def update_home(rows):
    ...
    out = [
        ...
        "",
        f"> 更新时间：{DATE}；当前正式文章 {len(rows)} 篇。",
        "> 发布状态：全部已整合入库；首页和总索引使用文章文件名前缀编号，下架或删除后不自动重排；只有明确批准的一次性迁移才按《编号变更记录》执行；《编号变更记录》保留旧号到新号的追溯关系。",
        "",
        "## 从哪里开始",
        "",
        ...
        "- [项目结构与文件归位说明](<../07-资料与流程/项目结构与文件归位说明.md>)：查找文章、素材、流程、脚本与临时文件的归属位置。",
        "",
        "## 按文章编号",
        "",
    ]
...
def update_agents_rule():
    p = ROOT / "AGENTS.md"
    text = p.read_text(encoding="utf-8")
    new = '- 新增文章时同步更新 `README.md`、`00-知识库导航/知识库总索引.md`、`分类索引.md`、`系列索引.md`、`标签索引.md`、`07-资料与流程/文章资产登记表.md` 和 `07-资料与流程/发布记录.md`；默认使用文件名前缀编号。删除、下架或转为草稿不修改既有文章文件名、图片和元数据；只有已批准的一次性迁移，才按《编号变更记录》同步这些内容。'
