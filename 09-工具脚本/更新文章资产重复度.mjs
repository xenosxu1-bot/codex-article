...
const defaultAssetPath = path.join(repoRoot, '07-资料与流程', '文章资产登记表.md');
...
function toMarkdown(report) {
  ...
  for (const row of report.rows) {
    lines.push(`| ${row.id} | ${row.title} | ${row.max_repeat_percent}% | ${row.body_similarity_percent}% | ${row.title_similarity_percent}% | ${row.match.id} ${row.match.title} | ${row.result} |`);
  }
  return lines.join('\n') + '\n';
}
